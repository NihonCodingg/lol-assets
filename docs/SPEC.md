...
| **RNF-13** | Integridade **verificável, não garantida** | O `sha256` do índice descreve os bytes medidos na indexação. Como quem serve é a fonte, ele vira **detector** — e só nas fontes de bytes estáveis: o cdragon passa pelo Cloudflare Polish e entrega bytes diferentes conforme o cache, com a mesma arte ([ADR 0019](adr/0019-o-sha256-do-cdragon-nao-confere-o-download.md)). O front compara o `sha256` quando `isByteStable(source)` e, nas outras fontes, formato e dimensões; avisa quando diverge, em vez de impedir | Teste: `sha256` divergente numa fonte estável produz aviso visível e não bloqueia o download; no cdragon, não produz aviso |
